...
export async function combinedAuth(req: CombinedAuthRequest, res: Response, next: NextFunction) {
  // Try API key first
  ...
  if (apiKeyHeader && apiKeyHeader.length >= 10) {
    try {
      ...
      if (record && !record.revoked_at) {
        req.apiKey = { id: record.id, prefix: record.prefix, name: record.name };
